...
async function abiertaRuleta(req, res) {
  try {
    ...
    res.json({ 
      msg: 'Mesa de ruleta está abierta' 
    });
    ...
    res.status(500).json({ 
      ok: false, 
      msg: 'No se pudo abrir la mesa de ruleta' 
    });
  }
}
  ...
  try {
    ...
    if (tipo !== 'numero') {
      return res.status(400).json({ 
        error: 'Solo se permiten apuestas por número'
       });
    }
    if (valor < 0 || valor > 36) {
      return res.status(400).json({ 
        error: 'Número fuera de rango (0-36)' 
      });
    }
    if (monto > 10000) {
      return res.status(400).json({ 
    ...
    if (!ruleta || ruleta.estado !== 'abierta') {
      return res.status(400).json({ 
        error: 'Ruleta no disponible para apostar' 
      });
    }
    ...
    res.status(500).json({ 
      ok: false, 
      msg: 'No se pudo registrar la apuesta' 
    });
  }
}
...
async function cerradaRuleta(req, res) {
  try {
    ...
    if (!ruleta || ruleta.estado !== 'abierta') {
      return res.status(400).json({ 
        error: 'Ruleta no disponible o ya cerrada' 
      });
    }
    ...
    res.json({
      ...
      console.error(error);
      res.status(500).json({ 
        error: 'Error al cerrar la ruleta' 
      });
    }
  }
